...
const AdminPanel = ({
  ...
  const confirmApprove = () => {
    if (modal.type === "deposit") {
      ...
      const tx = admin.withdraws.find(tx => tx._id === modal.txId);
      if (!tx) return;
      if (tx.userId.balance < tx.amount) {
        setAlertModal({ show: true, message: `Cannot approve withdrawal: User "${tx.userId.name}" has insufficient balance.` });
        ...
        <Row>
          <Col md={2}>
            <div style={sidebarStyle}>
              <div style={tabStyle("deposits")} onClick={() => setActiveTab("deposits")}><span style={iconStyle}>💰</span> Deposit Requests</div>
              <div style={tabStyle("withdraws")} onClick={() => setActiveTab("withdraws")}><span style={iconStyle}>🏦</span> Withdraw Requests</div>
              <div style={tabStyle("users")} onClick={() => setActiveTab("users")}><span style={iconStyle}>👥</span> Users</div>
              <div style={tabStyle("qr")} onClick={() => setActiveTab("qr")}><span style={iconStyle}>📷</span> QR Code</div>
            </div>
          </Col>
          <Col md={10}>
                ...
                <Card.Body>
                        ...
                        return (
                          <tr key={tx._id}>
                            ...
                            <td>
                              {tx.screenshot ? (
                                <a
                                  href={`${process.env.REACT_APP_API_URL}/uploads/${filename}?token=${localStorage.getItem("token")}`}
                                  target="_blank"
                                  rel="noreferrer"
                                >
                                  View
                  ...
                  <Pagination className="justify-content-center mt-3">
                    {[...Array(totalPages(filteredDeposits))].map((_, num) => (
                      <Pagination.Item key={num + 1} active={currentPage === num + 1} onClick={() => setCurrentPage(num + 1)}>{num + 1}</Pagination.Item>
                    ))}
                  </Pagination>
                </Card.Body>
              </Card>
  ...
  <Card style={cardStyle} className="mb-4">
    <Card.Body>
      ...
      <div className="mt-3">
        ...
        {admin.qrCodeUrl ? (
          <img 
            src={`http://localhost:5051${admin.qrCodeUrl}`} 
            alt="QR" 
            style={{ width: "200px" }} 
          />
        ) : (
      ...
      <Modal show={modal.show} onHide={closeModals} centered>
        ...
        <Modal.Body style={{ background: "#2c3e50", color: "#fff" }}>Are you sure you want to approve this {modal.type}?</Modal.Body>
      ...
      <Modal show={confirmUserModal.show} onHide={closeModals} centered>
        ...
        <Modal.Footer style={{ background: "#2c3e50" }}>
          <Button variant="secondary" onClick={closeModals}>Cancel</Button>
          <Button variant="danger" onClick={confirmUserAction}>Confirm</Button>
        </Modal.Footer>
      </Modal>
