...
export default function adminReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case APPROVE_DEPOSIT:
      return { ...state, deposits: state.deposits.filter(tx => tx._id !== action.payload) };
    case APPROVE_WITHDRAW:
      return { ...state, withdraws: state.withdraws.filter(tx => tx._id !== action.payload) };
    ...
    case UPDATE_QR_CODE:
      return { ...state, qrCodeUrl: action.payload }; // ✅ update new QR
    default:
      return state;
  }
}
